...
from src.data.comtrade_client import collect_comtrade_data, collect_russia_trade
    ...
    def test_columns_can_be_selected(self):
        with (
            ...
            result = collect_comtrade_data(
                ...
                hs_codes="72",
                periods="2020",
                columns=["refYear", "cmdCode", "reporterName", "partnerName"],
            )
        ...
        assert result["reporterName"].eq("Korea").all()
        assert result["partnerName"].eq("World").all()
